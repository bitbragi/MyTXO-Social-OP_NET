#!/usr/bin/env node
/**
 * Deploy ScribeRewards to OP_NET testnet.
 *
 * Prerequisites:
 ...
 *
 * Usage: node opnet/scripts/deploy-rewards.mjs
 *
 * Env vars written:
 *   REWARDS_CONTRACT_ADDRESS, REWARDS_CONTRACT_P2OP, NEXT_PUBLIC_REWARDS_ADDRESS
 */
...
async function main() {
    ...
    if (env.REWARDS_CONTRACT_ADDRESS) {
        console.log(`Rewards already deployed: ${env.REWARDS_CONTRACT_ADDRESS}`);
        console.log('Remove REWARDS_CONTRACT_ADDRESS from .env to redeploy.');
        process.exit(0);
    }

    const wasmPath = resolve(__dirname, '../contracts/build/ScribeRewards.wasm');
    ...
    appendEnv('REWARDS_CONTRACT_ADDRESS', contractHex);
    appendEnv('REWARDS_CONTRACT_P2OP', contractP2op);
    appendEnv('NEXT_PUBLIC_REWARDS_ADDRESS', contractHex);

    await waitForConfirmation(provider, revealResult.result, 'ScribeRewards deployment');

    console.log('\n===============================================');
    console.log('  ScribeRewards deployed!');
    console.log(`  Contract (hex):  ${contractHex}`);
    console.log(`  Contract (P2OP): ${contractP2op}`);
    console.log('');
    console.log('  NEXT STEPS:');
    console.log('  1. Fund rewards: node opnet/scripts/fund-rewards.mjs');
    console.log('  2. Exclude from tax: node opnet/scripts/setup-tax-exclusions.mjs');
    console.log('===============================================');
